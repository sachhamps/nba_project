# using the scikit learn library to experiment with neural nets
# experimenting with different features to which ones are best at indicating whether a team will make the playoffs
...
#training data - team statistics 1980-2011
training_data = pd.read_csv('NBA_train.csv')
#X = np.matrix([training_data.values[:,4] - training_data.values[:,5]]) #points - opponent points
X = np.matrix([training_data.values[:,3], training_data.values[:,4] - training_data.values[:,5]]) #wins and net points
X = X.T
y = np.array(training_data.values[:,2]) #1 if they made the playoffs, 0 if they did not make the playoffs
y = y.astype('int')
#plt.scatter(X[0],y)
#plt.show()
